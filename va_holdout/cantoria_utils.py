...
def voice_metrics(y_true, y_pred,
    ...
    y_true_melody = [np.array([0]) if freq < TRUE_THRES else np.array([freq]) for freq in y_true]
    y_pred_melody = [np.array([0]) if freq <= PRED_THRES else np.array([freq]) for freq in y_pred]
    ...
    melody_metrics =  mir_eval.melody.evaluate(timescale, np.array(y_true_melody).squeeze(),
                                               timescale, np.array(y_pred_melody).squeeze())
