...
for i in range(1, 6):
    ...
    kfold_sscs_f_scores = np.array([sop_metrics['F-Measure'].to_numpy(), alto_metrics['F-Measure'].to_numpy(), ten_metrics['F-Measure'].to_numpy(), bass_metrics['F-Measure'].to_numpy()])

    mix_df, sop_df, alto_df, ten_df, bass_df = cantoria_utils.compute_kfold_metrics(train=2, fold=i)
    kfold_cantoria_f_scores = np.array([sop_df['F-Measure'].to_numpy(), alto_df['F-Measure'].to_numpy(), ten_df['F-Measure'].to_numpy(), bass_df['F-Measure'].to_numpy()])

    #boxplot_generation(kfold_sscs_f_scores, model_name, 'Fold ' + str(i), 'SSCS')
    #boxplot_generation(kfold_cantoria_f_scores, model_name, 'Fold ' + str(i), 'Cantoría')
    evaluation_boxplots_generation(sop_metrics, alto_metrics, ten_metrics, bass_metrics, model_name, 'Fold ' + str(i), 'SSCS')
    evaluation_boxplots_generation(sop_df, alto_df, ten_df, bass_df, model_name, 'Fold ' + str(i), 'Cantoría')
    voicing_boxplots_generation(sop_metrics, alto_metrics, ten_metrics, bass_metrics, model_name, 'Fold ' + str(i), 'SSCS')
    voicing_boxplots_generation(sop_df, alto_df, ten_df, bass_df, model_name, 'Fold ' + str(i), 'Cantoría')